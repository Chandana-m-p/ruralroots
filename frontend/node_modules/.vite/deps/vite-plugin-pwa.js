...
import {
  ...
  slash,
  version
} from "./chunk-LDRY3LMT.js";
import {
  __require,
  cyan,
  yellow
} from "./chunk-KAH4YNEF.js";
import {
  require_node_crypto
...
var _dirname = typeof __dirname !== "undefined" ? __dirname : (0, import_node_path2.dirname)((0, import_node_url.fileURLToPath)(import.meta.url));
async function loadWorkboxBuild() {
  try {
    const workbox = await import("./build-KOLKX5WS.js");
...
async function generateRegisterSW2(options, mode, source = "register") {
  ...
  const content = await import_node_fs2.promises.readFile((0, import_node_path2.resolve)(_dirname, `client/${mode}/${source}.js`), "utf-8");
...
self.addEventListener('activate', (e) => {
  self.registration.unregister()
    ...
    `;
    await import_node_fs2.promises.writeFile(options.swDest.replace(/\\/g, "/"), selfDestroyingSW, { encoding: "utf8" });
    return {
      count: 1,
...
async function generateInjectManifest(options, viteOptions) {
  ...
  const [workbox, buildSW] = await Promise.all([
    loadWorkboxBuild(),
    import("./vite-build-YBKNZKXD-6WC6TDKT.js").then(({ buildSW: buildSW2 }) => buildSW2)
  ]);
...
function buildManifestEntry(publicDir, url) {
  return new Promise((resolve5, reject) => {
    ...
    const stream = import_node_fs3.default.createReadStream((0, import_node_path3.resolve)(publicDir, url));
    stream.on("error", (err) => {
      reject(err);
    });
    ...
    async load(id) {
      ...
      if (!options.disable && options.devOptions.enabled) {
        if (options.strategies === "injectManifest" && !options.selfDestroying) {
          ...
          if (id === swSrc) {
            let content = await import_node_fs4.promises.readFile(options.injectManifest.swSrc, "utf-8");
        ...
        if (id.endsWith(swDevOptions.swUrl)) {
          ...
          const swDest = (0, import_node_path4.resolve)(globDirectory, "sw.js");
          if (!swDevOptions.swDevGenerated) {
            let suppressWarnings;
            if (options.devOptions.suppressWarnings === true) {
              suppressWarnings = normalizePath((0, import_node_path4.resolve)(globDirectory, "suppress-warnings.js"));
            ...
            filePaths.forEach((we) => {
              const name = (0, import_node_path4.basename)(we);
              if (name !== "sw.js")
                swDevOptions.workboxPaths.set(normalizePath(`${options.base}${name}`), we);
            });
            if (suppressWarnings) {
              swDevOptions.workboxPaths.set(
                normalizePath(`${options.base}${(0, import_node_path4.basename)(suppressWarnings)}`),
                suppressWarnings
              );
            }
...
async function resolveDevDistFolder(options, viteConfig) {
  return options.devOptions.resolveTempFolder ? await options.devOptions.resolveTempFolder() : (0, import_node_path4.resolve)(viteConfig.root, "dev-dist");
}
async function createDevRegisterSW(options, viteConfig) {
  if (options.injectRegister === "script" || options.injectRegister === "script-defer") {
    ...
    const registerSW = (0, import_node_path4.resolve)(devDist, FILE_SW_REGISTER);
    if (!swDevOptions.registerSWGenerated) {
      await import_node_fs4.promises.writeFile(registerSW, generateSimpleSWRegister(options, true), { encoding: "utf8" });
...
function resolveSwPaths(injectManifest, root, srcDir, outDir, filename) {
  const swSrc = (0, import_node_path5.resolve)(root, srcDir, filename);
  if (injectManifest && (0, import_node_path5.extname)(filename) === ".ts" && import_node_fs5.default.existsSync(swSrc)) {
    const useFilename = `${filename.substring(0, filename.lastIndexOf("."))}.js`;
    return {
      swSrc,
      swDest: (0, import_node_path5.resolve)(root, outDir, useFilename),
      useFilename
    };
  }
  return {
    swSrc,
    swDest: (0, import_node_path5.resolve)(root, outDir, filename)
  };
}
async function resolveOptions(ctx) {
  ...
  const outDirRoot = (0, import_node_path5.resolve)(root, outDir);
  ...
  return {
    ...
    async configResolved(config) {
      ...
      if (ctx.options.pwaAssets && !ctx.options.pwaAssets.disabled) {
        ctx.pwaAssetsGenerator = import("./generator-EZTYL4FH-VDSZWAKO.js").then(({ loadInstructions }) => loadInstructions(ctx)).catch((e) => {
          console.error([
            "",
            cyan(`PWA v${ctx.version}`),
